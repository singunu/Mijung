...
for item_category_code in item_category_code_list:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for item in root.findall('.//item'):
                        ...
                        # 데이터가 이미 존재하는지 확인
                        if not any((d['date'] == regday and d['item_name'] == item_name) for d in data_list):
                            data = {
                                'date': regday,
                                'item_name': item_name,
                                'item_code': item_code,
    ...
    for item in df['item_name'].unique():
        ...
        if not df_item.empty:
            ...
            file_name = f'C:/Users/SSAFY/Desktop/분석/시계열분석/축산제외 전체시계열데이터/{item_code}_{item}_{start_year}_{end_year}_data.xlsx'
